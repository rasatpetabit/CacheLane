# CacheLane hang remediation — spec v3

**Supersedes** v1 and v2 (both kept as the record). Product of a 5-lens independent
investigation plus **two rounds** of rigorous cross-vendor adversarial review (2 reviewers per
round; round 1 `rework` — 1 major + 3 minor; round 2 `rework` — 5 major + 1 minor).

**Status:** awaiting spec approval. No production code written. **Date:** 2026-07-31
**Repo:** `/srv/dev/ai/cachelane` @ `main` = `3883ee7`

Both review rounds targeted the **design and gate** sections only. **No reviewer in either
round challenged the root cause (§1) or its evidence.**
...
Unchanged and still confirmed across both rounds: the root cause (§1), M1 (§2), the corrections
to the original brief (§3), and the incomplete-bypass finding (§4).
...
3. **Layer 3 — hysteresis via a deterministic bucket-derived cutoff (M7).**
   ...
   request's own size to a coarse band — e.g. `band = floor(log2(total_prompt_tokens))` — and
   derive the effective cutoff `K_eff = f(band)` from that band alone. Then:
   - the same request always yields the same band, hence the same elided set, **re-materialized
     identically on every request** (no reappearance);
   - the elided set only grows when the conversation crosses into a higher band, so the prefix
     is invalidated **once per band change (~per doubling)** rather than once per turn;
   - no component remembers anything. Statelessness is preserved by construction.

   `f` must be monotone non-decreasing in `band` so I2 (once elided, always elided) survives
   band changes. Band boundaries need a small dead-zone (hysteresis *in the function*, e.g.
   round down with a sticky margin derived from the request) so a conversation oscillating
   around a boundary does not flip sets — this must also be a pure function of the request, not
   remembered.
...
- **Metric:** per-session `cache_read / (cache_read + input)` from the live `turns` table, plus
  absolute billed input tokens — a ratio can improve while total cost rises.
- **Pass: mutation-on beats mutation-off on both**, over ≥ 20 sessions reaching the elision
  depth of §9's primary gate. Otherwise do not re-enable.

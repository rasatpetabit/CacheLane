# CacheLane hang remediation — spec v2

**Supersedes** `spec-hang-remediation-v1.md` (kept as the record). v2 folds in a rigorous
cross-vendor adversarial review (2 reviewers, `rework`: 1 major + 3 minor) **and one finding
that review led me to, which overturns v1's central remediation.**
...
| # | Change | Source |
|---|---|---|
| **M1** | **The un-stubbing bug is load-bearing. Fixing it naively disables pruning entirely.** See §2. This is new, and it inverts v1's Step 4. | Found while checking the reviewer's major finding |
...
| **M3** | The multi-turn closed-loop test is rewritten: feed the **original growing history** forward, not the forwarded body. A transparent proxy's client never sees the mutated body. | Reviewer #2 (major) |
| **M4** | The mutation experiment in v1 §3 was **confounded** — it disabled `pruner.enabled` *and* `mutation_enabled` together. | Reviewer #1 (minor) |
...
Unchanged from v1 and still confirmed: the root cause (§1), the corrections to the original
brief (v1 §2), and the incomplete-bypass finding (v1 §3).
...
3. **Layer 3 — hysteresis.** Elision *is* cache invalidation. Batch-convert all eligible blocks
   on a threshold crossing (prompt > X tokens), not per turn — "one invalidation per doubling"
   rather than per turn. The only shape where the arithmetic can close.
4. **Layer 4 — escrow, or stop lying.** `blocks` has **no content column**
   ...
   **structurally impossible**. Store originals keyed by `sha256(content)` with real TTL, or
   remove the promise from `formatStubText` (`pruner/stubs.ts:13`). Make `expand` non-mutating
   either way.
...
*best*: LiteLLM (heavy pruning, 6,278 prune events) runs **37–41%** cache-read ratio; the Claude
lane (`marker_strategy: passthrough`, 159–564 events) runs **84–100%**.
...
45 ms/block/turn), the economics falsification, and the fabricated savings telemetry, the
defensible options are, in order:

**A. Fix the hang, leave pruning off.** Steps 1–3 + 5 below. CacheLane becomes a recording,
cache-hint-injecting, observable proxy. No mutation, no elision. Low risk, and the passthrough
lane already outperforms the mutating one 84–100% vs 37–41%.

**B. A, then rebuild pruning as Layer 1 behind an experiment arm,** gated on §9's economics
gate. Only ship it if mutation-on beats mutation-off on the same workload.
...
(d) A `server.on('clientError')` handler — Node's default destroys pre-parse sockets silently,
so a failed request never even logs `incoming`.
...
- **Monotonicity under append:** if block X is elided for prefix *P*, it is elided for every
  *P + suffix*. No block ever un-elides.
- **Bounded cost:** per-turn transform time is `O(elided) × <1 ms`, and total handler time is
  flat in elided count (Gate 1).
...
**Gate 4 — connection lifetime (M6-corrected).** v1 asserted "no FIN within 120 s on an idle
socket", which wrongly treats normal idle expiry as a failure and would encourage unbounded idle
socket retention. Replace with:

- **No in-flight termination:** the proxy never closes a socket with a request in flight or an
  SSE stream open, at any idle-timer boundary.
- **Advertisement matches behaviour:** the `Keep-Alive: timeout=N` header equals the effective
  `server.keepAliveTimeout`, so pooling clients can honour it. **Today the server advertises
  `timeout=5` and FINs at ~6.0 s** (verified: `:7333` 6.008 s, `:7332` 6.007 s, while
  LiteLLM:4000 was still open at 20 s).
- **Timeout exceeds the client reuse window:** set `keepAliveTimeout` above the observed
  inter-turn idle distribution (measured mean 5,031 ms, 5.8% > 5 s — i.e. today's default sits
  *exactly* on the distribution), with `headersTimeout` strictly greater.

**Gate 5 — economics, before mutation is ever re-enabled.** Per-session
`cache_read / (cache_read + input)` from the live `turns` table, mutation-on vs mutation-off on
the same workload, using the de-confounded config from §4 (**pruner on, mutation varied**).
**Re-enable only if on > off.** Today: 37–41% (mutating) vs 84–100% (near-passthrough) — a
...
| R-13 | LOW | **The stub text advertises a retrieval that cannot happen** (Layer 4), and `expand` side-effects `is_stub=0`, which any prefix-stable design must forbid. | Enable escrow or remove the promise; make `expand` non-mutating. |
